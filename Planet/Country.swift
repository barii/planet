...
extension Country {
    ...
    public static func all(locale: Locale = .current) -> [Country] {
        ...
        for region in Locale.Region.isoRegions {
            guard let countryName = (locale as NSLocale).displayName(forKey: NSLocale.Key.countryCode, value: region.identifier) else {
                continue
            }
            
            guard let callingCode = callingCodes[region.identifier] else {
                continue
            }
            
            let country = Country(name: countryName, isoCode: region.identifier, callingCode: "+\(callingCode)")
